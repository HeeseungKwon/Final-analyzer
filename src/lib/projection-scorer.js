...
export function calculateMarketProjectionScore(
  ...
  } else if (market.includes("2+ Total") || market === "total_bases") {
    expectedValueScore = normalizeToSlate(
      projectedStats.expectedTotalBases,
      slateNorms?.minTB2 ?? 0,
      slateNorms?.maxTB2 ?? 3
  ...
  } else if (market === "strikeouts") {
    // For pitchers, just use 50 (neutral)
    expectedValueScore = 50;
  } else {
    // Hits markets
    expectedValueScore = normalizeToSlate(
  ...
  } else if (market.includes("Total") || market === "total_bases") {
    metricsScore = calculatePowerProfileScore(ctx);
  } else if (market === "strikeouts") {
    metricsScore = 50; // Neutral for pitchers
  } else {
    // Hits markets
...
/**
 * Get all market rankings - includes ALL markets
 */
export function getAllMarketRankings(predictions) {
  // Complete list of all possible markets
  const markets = [
    "hit_2",        // 2+ Hits
    "total_bases",  // TB O1.5
    "home_run",     // Home Run
