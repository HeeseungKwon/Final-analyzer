const db = globalThis.__B44_DB__ || { auth:{ isAuthenticated: async()=>false, me: async()=>null }, entities:new Proxy({}, { get:()=>({ filter:async()=>[], get:async()=>null, create:async()=>({}), u[...]
        ...
        try {
          ...
          const scoredPredictions = await Promise.all(modernScores.map(async (s) => {
            ...
            let predRow = {
              ...
              verdict_note: `Model ${(edgeMetrics.modelProbability * 100).toFixed(1)}% vs Market ${(edgeMetrics.impliedProbability * 100).toFixed(1)}% | Edge: ${(edgeMetrics.edge * 100).toFixed(1)}%`,
            };
