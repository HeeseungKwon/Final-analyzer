                ...
                <TableRow key={p.id || idx}>
                  ...
                  <TableCell className="text-right text-sm">
                    {market.includes("HR") || market.includes("home_run")
                      ...
                      : market.includes("2+ Total") || market === "total_bases"
                      ? Number(p.expected_total_bases ?? 0).toFixed(2)
...
export default function Projections() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // DEBUG: Log all unique markets
      const uniqueMarkets = [...new Set(withScores.map(p => p.market))];
      console.log('[Projections] Date:', date);
      console.log('[Projections] Total predictions:', withScores.length);
      console.log('[Projections] Unique markets:', uniqueMarkets);
      console.log('[Projections] Raw predictions:', withScores.slice(0, 5));
  ...
  const rankings = useMemo(() => {
    const allRankings = getAllMarketRankings(predictions);
    console.log('[Projections] Rankings from getAllMarketRankings:', Object.keys(allRankings));
    return allRankings;
  }, [predictions]);
      ...
      {!isLoading && predictions.length > 0 && (
        <>
          ...
          <Card className="mb-6 bg-muted/50">
            <CardContent className="pt-6">
              <div className="text-xs text-muted-foreground space-y-1">
                ...
                <div>Markets found: {Object.keys(rankings).length}</div>
                <div>Markets: {Object.keys(rankings).join(", ") || "None"}</div>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-6">
            {Object.entries(rankings).length > 0 ? (
              ...
              <Card>
                <CardContent className="py-10 text-center text-sm text-muted-foreground">
                  No market rankings available. Check database for predictions.
                </CardContent>
              </Card>
            )}
          </div>
